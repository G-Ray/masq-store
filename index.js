...
import common from 'masq-common/index'
...
/**
* The list of required parameters for an app, used wich error.checkObject function
*/
const requiredParameterApp = ['name', 'url']
/**
* The list of required parameters for an user, used wich error.checkObject function
*/
...
class MasqStore {
  ...
  checkCurrentUser () {
    if (!this._currentUserId) {
      throw error.generateError(error.ERRORS.NOLOGGEDUSER)
    }
  }
  async init () {
  ...
  async createUser (user) {
    error.checkObject(user, requiredParameterUser)
    const users = await this.publicStore.getItem('userList')
    if (users[user.username]) {
      throw error.generateError(error.ERRORS.EXISTINGUSERNAME)
    }
    // Add a uuid.
    user._id = common.generateUUID()
    users[user.username] = user
    await this.publicStore.setItem('userList', users)
    this.profileStore = await this.initInstance(user._id)
  ...
  async signIn (username) {
    if (!username || username === '') {
      throw error.generateError(error.ERRORS.NOUSERNAME)
    }

    const users = await this.publicStore.getItem('userList')
    if (!users[username]) {
      throw error.generateError(error.ERRORS.USERNAMENOTFOUND)
    }
    this._currentUserId = users[username]._id
    // Initialise the profile instance
    if (!this.profileStore) {
      this.profileStore = await this.initInstance(this._currentUserId)
    }
  }
 ...
 */
  ...
  async initInstance (id) {
    const instance = new Store(id, this.storage)
    await instance.init()
    return instance
  }
 ...
 */
  async addApp (appMeta) {
    this.checkCurrentUser()
    error.checkObject(appMeta, requiredParameterApp)
 ...
 */
  async deleteApp (url) {
    this.checkCurrentUser()
    if (!url || url === '') {
      throw error.generateError(error.ERRORS.WRONGPARAMETER)
    }
    let user = await this.getProfile()
 ...
 */
  async updateApp (appMeta) {
    this.checkCurrentUser()
    error.checkObject(appMeta, ['url'])
    const appId = this.deriveAppId(appMeta.url)
    let user = await this.getProfile()
    if (!user.appList[appId]) {
      throw error.generateError(error.ERRORS.NOEXISTINGKEY)
    }
    appMeta.id = appId
    user.appList[appId] = appMeta
    // await this.profileStore.setItem(this._currentUserId, user)
